...
int main(int argc, char **argv){
  ...
  ros::Publisher pub_accel = n.advertise<sensor_msgs_ext::accelerometer>("imu/accelerometer", 2);
  ros::Publisher pub_mag = n.advertise<sensor_msgs_ext::magnetometer>("imu/magnetometer", 2);
  ...
  while (ros::ok()){
    ...
    sensor_msgs_ext::accelerometer data_imu_4_calib;
    sensor_msgs_ext::magnetometer data_mag;
    //sensor_msgs::MagneticField data_mag;

    //data_mag.header.stamp = ros::Time::now();
    data_imu.header.stamp = ros::Time::now();//data_mag.header.stamp;
    ...
    // 4 calibration
    data_imu_4_calib.x = InBuffer[0]*conversion_acce;
    data_imu_4_calib.y = InBuffer[1]*conversion_acce;
    data_imu_4_calib.z = InBuffer[2]*conversion_acce;
    ...
    data_mag.x = InBuffer[6]*M_factor;
    data_mag.y = InBuffer[7]*M_factor;
    data_mag.z = InBuffer[8]*M_factor;

    pub_imu.publish(data_imu);

    pub_mag.publish(data_mag);
    pub_accel.publish(data_imu_4_calib);
